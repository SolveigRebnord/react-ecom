    ...
    <>
      <section className="md:mx-24 lg:w-1/2 lg:m-auto my-12 md:my-28 lg:my-32">
        ...
        {cartProducts && cartProducts != {} ? (
          <>
            <div className="flex flex-col ">
              {cartProducts.map((product) => {
                ...
                discountTotal =
                  product.product.discountedPrice * product.amount;
                ...
                return (
                  <div
                    key={product.id}
                    className=" flex flex-col border-t border-gray-200 py-8 px-8 last-of-type:border-b md:px-0"
                  >
                    ...
                    <div className="flex flex-row justify-between items-end">
                      <div className="flex flex-col gap-4">
                        <div className="flex flex-row items-center gap-6">
                          <button
                            onClick={() =>
                              dispatch(plusQuantity(product.product.id))
                            }
                          >
                            <img src="/plus.svg" className="w-8" />
                          </button>
                          <p className="flex flex-row items-baseline gap-1">
                            x <span className="text-xl">{product.amount}</span>
                          </p>
                          <button
                            onClick={() =>
                              dispatch(minusQuantity(product.product.id))
                            }
                          >
                        ...
                        <PlainBtn
                          ...
                          onClick={() =>
                            dispatch(DeleteProductFromCart(product.product.id))
                          }
                        >
                          <img src="/x.svg" className="w-4" />
                          Remove
                        </PlainBtn>
                      </div>
                      <div className="">
                        <p className="pPrize">
                          {round(product.product.price * product.amount, 2)}{" "}
                          <PNOK>NOK</PNOK>
                        </p>
                        <p className="pPrize">
                          -{" "}
            ...
            <section className="mx-10 mt-8 md:mt-12">
              ...
              <div className="flex flex-row items-end justify-between">
                ...
                <p className="text-xl">
                  {round(totalProducts - diff, 2)} <PNOK>NOK</PNOK>
                </p>
              </div>
            </section>
          ...
          <div className="text-center flex flex-col gap-6 p-8 w-1/2 m-auto md:mb-96 md:mt-32 md:p-14 md:border md:border-gray-200 md:shadow-md rounded-md">
