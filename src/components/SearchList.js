...
function List(props) {
    ...
    return (
        ...
        <div className='w-full relative md:w-1/2 mx-auto'><p className='absolute shadow-lg rounded-md bg-white p-4 z-20 left-0 -top-12 w-full text-sm'>No hits..</p></div> 
        : <div className='relative w-full md:w-1/2 mx-auto'>
            <ul className='absolute shadow-lg rounded-md bg-white p-4 z-20 left-0 -top-12 w-full flex flex-col gap-8'> 
             {filteredData.map((item) => (
                 <Link to={`/product/${item.id}`}> 
                <li key={item.id} className='flex flex-row justify-between bg-mainGrey p-4'>
